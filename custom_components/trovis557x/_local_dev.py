...
def apply_local_trovis_modbus_override() -> None:
    ...
    _LOGGER.warning(
        "TROVIS stack: trovis-modbus=%s, modbus-connection=%s, tmodbus=%s",
        version("trovis-modbus"),
        version("modbus-connection"),
        version("tmodbus"),
    )
    ...
    if not src_path.exists():
        _LOGGER.warning(
            "Local trovis-modbus override configured but path does not exist: %s",
            src_path,
        )
        return
    ...
    if src_path_str not in sys.path:
        ...
        _LOGGER.warning(
            "Using local trovis-modbus checkout from %s",
            src_path_str,
        )
